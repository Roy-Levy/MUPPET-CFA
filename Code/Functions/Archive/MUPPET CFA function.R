...
") # closes the model as string
  ...
  if(model.check){
    ...
    # * * * Select PPMC results for discrepancy measure of interest
    if(model.has.outcomes){
      external.variable.prefix = "y"
      num.external.vars = K
    }  
    
    if(model.has.covariates){
      external.variable.prefix = "w"
      num.external.vars = H
    } 
    ...
    # loop over pairs of indicator and external variable and compute and name p-values
    p.values.local=NULL
    
    j=1
    which.external.var=1
    for(j in 1:J){
      for(which.external.var in 1:num.external.vars){
        ...
        temp1 <- temp1[ , grep(
          paste0(external.variable.prefix, which.external.var),
          colnames(temp1),
          value=TRUE
        )
        ]
        
        realized.values <- temp1[ , grep(
          paste0("realized"),
          colnames(temp1),
          value=TRUE
        )
        ...
        p.value <- mean(postpred.values >= realized.values)
        
        p.values.local <- c(p.values.local, p.value)
        names(p.values.local)[length(p.values.local)] <- paste0("x",j, "_", external.variable.prefix, which.external.var)
      } # closes loop over external variables
    } # closes loop over indicators
    
    resid.cor.p.values <- p.values.local
    ...
    for(j in 1:(J-1)){
      for(jj in (j+1):J){
        ...
        for(which.external.var in 1:num.external.vars){
          ...
          p.values.local <- c(p.values.local, p.value)
          names(p.values.local)[length(p.values.local)] <- paste0("x",j, "_", "x",jj, "_", external.variable.prefix, which.external.var)
        } # closes loop over external variables
      } # closes 2nd loop over indicators
    } # closes loop over indicators
    
    ICDM.p.values <- p.values.local
